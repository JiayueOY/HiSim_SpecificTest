...
class GenericHeatPumpController(cp.Component):
    ...
    This controller takes data from other components and sends signals to the heat pump
    ...
    offset: float
        Temperature offset to compensate for hysteresis correction due to building temperature change
    mode : int
    ...
    def __init__(
        ...
        self.state_channel: cp.ComponentOutput = self.add_output(
            ...
            lt.LoadTypes.ANY,
            lt.Units.ANY,
            output_description=f"Description for {self.State} will follow here.",
        )
    ...
    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if force_convergence:
            ...
            if (hours == 9 and minutes >= 0) or (hours > 9 and hours < 16) or (hours == 16 and minutes < 0):
                if (hours == 10 and minutes >= 0) or (hours > 10 and hours < 14) or (hours == 14 and minutes < 0):
