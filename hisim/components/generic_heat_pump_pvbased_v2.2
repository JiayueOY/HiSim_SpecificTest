...
class GenericHeatPump(cp.Component):
    ...
    def __init__(
        ...
        # self.dhi_channel: cp.ComponentOutput = self.add_output(
        #     self.component_name,
        #     self.DiffuseHorizontalIrradiance,
        #     lt.LoadTypes.IRRADIANCE,
        #     lt.Units.WATT_PER_SQUARE_METER,
        ...
        # )
        #
        # self.ghi_channel: cp.ComponentOutput = self.add_output(
        #     self.component_name,
        #     self.GlobalHorizontalIrradiance,
        #     lt.LoadTypes.IRRADIANCE,
        #     lt.Units.WATT_PER_SQUARE_METER,
    ...
    def get_default_connections_from_weather(self) -> List[cp.ComponentConnection]:
        ...
        # Add TemperatureOutside connection
        connections.append(
        ...
        # # Add DiffuseHorizontalIrradiance connection
        # connections.append(
        #     cp.ComponentConnection(
        #         GenericHeatPump.DiffuseHorizontalIrradiance,
        #         weather_classname,
        #         Weather.DiffuseHorizontalIrradiance,
        #     )
        # )
        #
        # # Add GlobalHorizontalIrradiance connection
        # connections.append(
        #     cp.ComponentConnection(
        #         GenericHeatPump.GlobalHorizontalIrradiance,
        #         weather_classname,
    ...
    def get_default_connections_heatpump_controller(
        ...
        #         GenericHeatPump.DiffuseHorizontalIrradiance,
        #         controller_classname,
        #         GenericHeatPumpController.DiffuseHorizontalIrradiance,
        #     )
        # )
        #
        # # Add GlobalHorizontalIrradiance connection
        ...
        #         GenericHeatPump.GlobalHorizontalIrradiance,
        #         controller_classname,
        #         GenericHeatPumpController.GlobalHorizontalIrradiance,
        #     )
        # )
        return connections
...
class GenericHeatPumpController(cp.Component):
    ...
    def get_default_connections_from_building(self) -> List[cp.ComponentConnection]:
        """Get default connections from the building."""
    ...
    def get_default_connections_from_electricity_meter(self) -> List[cp.ComponentConnection]:
        """Get default connections from the electricity meter."""
    ...
    def build(
        ...
        This function sets important constants and parameters for the calculations.
        """
    ...
    def write_to_report(self) -> List[str]:
        """Write important variables to the report."""
        return self.heatpump_controller_config.get_string_dict()

    def i_simulate(self, timestep: int, stsv: cp.SingleTimeStepValues, force_convergence: bool) -> None:
        """Simulate the heat pump controller."""
        if force_convergence:
            ...
            time_in_minutes = timestep * self.my_simulation_parameters.seconds_per_timestep / 60

            hours = int(time_in_minutes // 60) % 24
            minutes = int(time_in_minutes % 60)

            if (hours == 9 and minutes >= 0) or (hours > 9 and hours < 16) or (hours == 16 and minutes < 0):
                if dhi > 0 and ghi / dhi >= 2:
                    ...
                    if temperature_mean_old < max_temp:
                        ...
                        self.controller_heatpumpmode = 'off'
                elif hours == 16 and minutes == 0:
                    if self.controller_heatpumpmode == "heating" and temperature_mean_old > self.temperature_set_heating:
                        self.controller_heatpumpmode = 'off'
                else:
                    self.controller_heatpumpmode = 'off'
            else:
    ...
    def conditions(self, set_temperature: float) -> None:
        ...
        if self.controller_heatpumpmode == "heating":
            if set_temperature > maximum_heating_set_temperature:
                self.controller_heatpumpmode = "off"
                return
        if self.controller_heatpumpmode == "cooling":
            if set_temperature < minimum_cooling_set_temperature:
                self.controller_heatpumpmode = "off"
                return
        if self.controller_heatpumpmode == "off":
            if set_temperature < minimum_heating_set_temperature:
                self.controller_heatpumpmode = "heating"
                return
            if set_temperature > maximum_cooling_set_temperature:
                self.controller_heatpumpmode = "cooling"
                return

    def smart_conditions(self, set_temperature: float, electricity_input: float) -> None:
        """Set conditions for the heat pump controller in smart mode."""
